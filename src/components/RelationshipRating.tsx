    ...
    <div className="space-y-4">
      ...
      <div className="bg-white border border-gray-300 rounded-lg overflow-hidden">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-gray-900">How It Works — Closeness Levels</span>
          </div>
      ...
      {!hasSequenceData && allPairs.length > 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 text-sm text-amber-800">
          ...
          <p>
            You can still set each pair manually below. Or go back to Step 2C and add sequence
            numbers to your activities — this lets the tool auto-fill most closeness levels for you.
          </p>
        </div>
      )}
          ...
          <div className="bg-white border border-gray-300 rounded-lg p-4 space-y-3">
            <div className="flex items-center justify-between">
              <div className="text-sm font-medium text-gray-900">
                Set: {ratedCount} of {allPairs.length} pairs
          ...
          <div className="bg-white border border-gray-300 rounded-lg overflow-hidden">
            ...
            {showFilters && (
              <div className="px-4 pb-4 space-y-4 border-t border-gray-200 pt-3">
                ...
                <div>
                  <label className="text-xs font-medium text-gray-700 mb-2 block">Filter by Closeness</label>
                  <div className="flex flex-wrap gap-2">
                    ...
                    <button onClick={() => setRatingFilter('not-rated')} className={`px-3 py-1.5 rounded text-xs font-medium transition-colors ${ratingFilter === 'not-rated' ? 'bg-yellow-500 text-white' : 'bg-gray-100 text-gray-700 hover:bg-yellow-100'}`}>
                      Not yet set ({getRatingFilterCount('not-rated')})
                    </button>
                  </div>
                </div>
      ...
      {allPairs.length === 0 && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 text-sm text-yellow-900">
          <p className="font-medium">No activity pairs to set</p>
          <p className="mt-1">Add at least 2 activities in Step 2C to set closeness between them.</p>
        </div>
      )}
    </div>
  );
